...
import dtsgen, { parseSchema } from 'dtsgenerator';
...
const dtsgenFunction = typeof dtsgen === 'function' ? dtsgen : dtsgen.default;

async function compileTsp(inputTspPath) {
...
async function generateServiceTsp(program) {
...
function extractTypes(swaggerSchema) {
  const regex =
...
async function generateTypesFromSwagger(outputTypesPath) {
  ...
  const swaggerSchema = await dtsgenFunction({
    contents: [parseSchema(swaggerYaml)],
  });
